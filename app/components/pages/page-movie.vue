<template>
  <div class="page-movie" v-if="movie.tmdbData">
    ...
    <router-link :to="{ name: 'searchMovie', query: { q: movie.tmdbData.title, movieId } }">
      <button>
        Manually match
      </button>
    </router-link>
    ...
    <h2>Files</h2>
    <button v-for="file in files" :key="file.id" @click="openFile(file.fullPath)">{{ file.name }}</button>
  </div>
</template>
